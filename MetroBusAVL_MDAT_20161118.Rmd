...
Samp <- select(NewTravTime,
  ...
  # sample_n(size = 10000) %>%
  # filter(Route == "X2" &
  #          StopID_Clean == 1000987
  #       ) %>%
  # filter(Route == "X2" |
  #          Route == "P6"
  #       ) %>%
  filter(Event_Time_Date == 4) %>% 
...
# %>% 
  # as.data.frame()
  # mutate(NextBus = Event_Time[min(which(Event_Time > SampTime)
  #                                )
  #                            ]
  #       )
...
# system.time(
# Testing3 <- sqldf("Select     *
#                              ,(Select     min(Event_Time)
#                                From       Samp            as s2
#                                Where      s2.Event_Time > s1.SampTime
#                               )                                        as NextBus
#                    From       Samp                                     as s1
#                    Where      s1.Route = s2.Route
#                       And     s1.StopID_Clean = s2.StopID_Clean
#          
#                   ") %>%
#   mutate(NB = as_datetime(NextBus,
#                           tz = "America/New_York"
#                          ),
#          WaitTime_Sec = as.numeric(NB - SampTime),
#          WaitTime_Min = WaitTime_Sec / 60
#         ) %>% 
#   arrange(Route,
#           StopID_Clean,
#           Event_Time
...
# View(Testing3)
# 
# View(filter(Testing3,
#             Event_Time == "2016-10-07 18:05:21")
#     )
# 
# View(arrange(Testing3,
#              Event_Time
#             )
#     )
...
#                                                       From       Samp
#                                                  )                            as s3
#                                            On    s2.Route = s3.Route
#                                            And   s2.StopID_Clean = s3.StopID_Clean
#                                            And   s3.Event_Time > s2.SampTime
# 
#                              )
#                   Group By   Route
#                             ,StopID_Clean
...
Testing_A <- sqldf("   Select               t1.*
  ...
  mutate(NB = as_datetime(NextBus,
          ...
          # str(Testing5)
          # View(head(Testing5, 500))
          # View(head(Samp, 500))



# Testing6 <- sqldf("     Select               RowNum_OG
#                                             ,Route
#                                             ,StopID_Clean
#                                             ,Event_Description
#                                             ,Event_Time
...
#                   "
#                  ) %>% 
#   mutate(NextBus_Time = as_datetime(NB2, tz = "America/New_York")
#         )
#   
# str(Testing6)
# View(Testing6)

# rm(Samp2, Testing3, Testing6)



# estimating approx 20min of runtime for all 2.8m records
system.time(
Testing4 <- select(Testing_A,
                   -NextBus
                  ) %>% 
  group_by(RowNum_OG) %>% 
  filter(NB == min(NB)
...
# NEED TO DO Event_Time_Day at 3, 6, 7


sss <- distinct(select(Testing5,
                       RowNum_OG
                      )
               )
...
#              StopID_Clean
#             ) %>% 
#     filter(Event_Time.y == Event_Time.y[min(which(Event_Time.y > SampTime)
#                                            )
#                                        ]
#           )
# 
...
#   
# Testing <- mutate(Samp,
#                   NB2 = vapply(Event_Time,
#                                function(x) min(which(x > SampleTime)
#                                               )
#                                       )
#                                   
#                  )
...
#            StopID_Clean
#           ) %>% 
#   mutate(Event_Time_Date_L1 = lag(Event_Time_Date)
#         )
#   mutate(NextBus = filter(NewTravTime,
#                           Route == ))
# 
#   mutate(WaitTime_Sec = ifelse(between(SampTime, MinTime, MaxTime),
#                                Event_Time - SampTime,
#                                NA
#                               ),
#          WaitTime_Min = WaitTime_Sec / 60
