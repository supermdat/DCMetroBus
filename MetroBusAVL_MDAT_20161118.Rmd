...
setwd("/Users/mdturse/Desktop/Analytics/Metro Bus AVL/Bus AVL Oct 2016")
...
StopID_Runs <- rle(AllDays_Sorted$Stop_ID)
...
AllDays_FirstStopID <- AllDays_Sorted[StopID_Runs$starts, ]

dim(AllDays_Sorted)
dim(AllDays_FirstStopID)

nrow(AllDays_Sorted) - nrow(AllDays_FirstStopID)
...
StartStopStats <- group_by(AllDays_BusDay,
                           StartStop_ID
                          ) %>% 
  summarise(TravelDistance_Mi_Mean = mean(TravelDistance_Mi, na.rm = TRUE),
            TravelDistance_Mi_Med = median(TravelDistance_Mi, na.rm = TRUE),
            TravelDistance_Mi_Cnt = sum(!is.na(TravelDistance_Mi)),
            
            TravelTimeSec_Mean = mean(TravelTime_Sec, na.rm = TRUE),
            TravelTimeSec_Med = median(TravelTime_Sec, na.rm = TRUE),
            TravelTimeSec_Cnt = sum(!is.na(TravelTime_Sec)),

            TravelTimeHr_Mean = mean(TravelTime_Hr, na.rm = TRUE),
            TravelTimeHr_Med = median(TravelTime_Hr, na.rm = TRUE),
            TravelTimeHr_Cnt = sum(!is.na(TravelTime_Hr))
           )

View(head(StartStopStats, 50))

```


Join StartStopStats with AllDays_BusDay
```{r}

AllDays_BusDay2 <- left_join(AllDays_BusDay,
                             StartStopStats,
...
# rm(StartStopStats)
str(AllDays_BusDay2)
summary(AllDays_BusDay2)

View(filter(AllDays_BusDay2,
            # is.na(TravelDistance_Mi) &
            StartStop_ID == "5002206--5002207"
            # TravelDistance_Mi_Mean = 0.1312205
            # TravelDistance_Mi_Med = 0.09659091
           )
    )

View(filter(AllDays_BusDay2,
            between(RowNum_OG, 1488, 1508)  # 1498
           )
    )
...
AllDays_BusDayRoute <- group_by(AllDays_BusDay,
                                Bus_ID,
                                Event_Time_Date,
                                Route
                               ) %>% 
