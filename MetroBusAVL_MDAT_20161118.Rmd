...
a <- filter(AllDays_BusDay,
       !is.na(TravelDistance_Mi)
      ) %>% 
  select(TravelDistance_Mi)

aq <- as.data.frame(quantile(a$TravelDistance_Mi, probs = c(0.05, 0.95)))
class(aq)
str(aq)
View(aq)

b_5 <- quantile(AllDays_BusDay$TravelDistance_Mi, probs = 0.05, na.rm = TRUE)
b_95 <- quantile(AllDays_BusDay$TravelDistance_Mi, probs = 0.95, na.rm = TRUE)


c <- select(AllDays_BusDay,
            TravelDistance_Mi
           ) %>% 
       filter(between(TravelDistance_Mi, b_5, b_95)
             ) %>% 
...
mean(AllDays_BusDay$TravelDistance_Mi, na.rm = TRUE)
mean(c$TravelDistance_Mi, na.rm = TRUE)

median(AllDays_BusDay$TravelDistance_Mi, na.rm = TRUE)
median(c$TravelDistance_Mi, na.rm = TRUE)

sum(!is.na(AllDays_BusDay$TravelDistance_Mi))
sum(!is.na(c$TravelDistance_Mi))

func_mean <- function(var, ...){
  q5 <- quantile(df$var, probs = 0.05, na.rm = TRUE)
  q95 <- quantile(df$var, probs = 0.95, na.rm = TRUE)
  
  df_90Pct <- select(df, var) %>% 
    filter(between(var, b_5, b_95)
          ) %>% 
    arrange(desc(var)
           )
  
  mean_val <- mean(df_90Pct$var, na.rm = TRUE)
  
  return(mean_val)
}

testing <- group_by(AllDays_BusDay,
                    StartStop_ID) %>% 
  do(TestMean = func_mean(data = ., var = TravelDistance_Mi)
           )


dynamicCalcGDD <- function(df, temp_var, group_var) {
        group_by_(df, group_var) %>%
        summarize_(var_mean = func_mean(temp_var)
                  )
    }

dynamicCalcGDD(df = AllDays_BusDay, group_var = "StartStop_ID", temp_var = TravelDistance_Mi)
...
Stats_StSt <- group_by(AllDays_BusDay,
                       StartStop_ID
                      ) %>% 
...
Stats_StSt_HrGrp <- group_by(AllDays_BusDay,
                             StartStop_ID,
                             Event_Time_HrGroup
                            ) %>% 
