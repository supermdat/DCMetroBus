...
# LL_Stats_Test <- data.table(head(LL_Stats, 100))
# setkey(LL_Stats_Test, RowNum)
# str(LL_Stats_Test)
# 
# View(LL_Stats_Test)
...
for(# i in 1:nrow(LL_Stats)
  ...
  lat <- filter(LL_Stats,
                RowNum == (i + 1950 + 1000 + 1000 + 1000 + 1000)
               ) %>%
    select(Lat_Med)
  ...
  # APIData7 <- fromJSON(paste0(url_1,
  # APIData8 <- fromJSON(paste0(url_1,
  # APIData9 <- fromJSON(paste0(url_1,
  # APIData10 <- fromJSON(paste0(url_1,
  # APIData11 <- fromJSON(paste0(url_1,
                             lat,
                             url_2,
                             lng,
                             url_3,
  ...
  message("Retrieving Zip Code ", (i + 1950 + 1000 + 1000 + 1000 + 1000)
         )
  ...
  # pages7[[i]] <- APIData7$postalCodes
  # pages8[[i]] <- APIData8$postalCodes
  # pages9[[i]] <- APIData9$postalCodes
  # pages10[[i]] <- APIData10$postalCodes
  # pages11[[i]] <- APIData11$postalCodes
...
# str(APIData2)
# head(APIData$postalCodes$postalCode)
# head(APIData)
# 
class(pages2)
# str(pages2)
...
head(pages2[[1]], 3)
tail(pages, 2)
pages$data.frame$APIData$postalCodes$postalCode
pages$APIData$postalCodes$postalCode
# head(pages[[1]], 2)
# 
# pages[[1]]$Products$List
# 
# pages[[1]]$Products$List$Id
# pages[[1]]$Products$List$Name
# 
# pages[[2]]$Products$List$Id
# pages[[2]]$Products$List$Name


##### Combine the lists into one

# APIData_All <- rbind.fill(pages)
# Zips <- rbind.pages(pages)
# Zips2 <- rbind.pages(pages2[sapply(pages2, length) > 0])
# Zips3 <- rbind.pages(pages3[sapply(pages3, length) > 0])
# Zips4 <- rbind.pages(pages4[sapply(pages4, length) > 0])
# Zips5 <- rbind.pages(pages5[sapply(pages5, length) > 0])
Zips6 <- rbind.pages(pages6[sapply(pages6, length) > 0])
# Zips7 <- rbind.pages(pages7[sapply(pages7, length) > 0])
# Zips8 <- rbind.pages(pages8[sapply(pages8, length) > 0])
# Zips9 <- rbind.pages(pages9[sapply(pages9, length) > 0])
# Zips10 <- rbind.pages(pages10[sapply(pages10, length) > 0])
# Zips11 <- rbind.pages(pages11[sapply(pages11, length) > 0])


pages[sapply(pages, length) <= 0]
pages2[sapply(pages2, length) <= 0]

str(Zips3)
tail(Zips)
View(Zips2)
# APIData_All2 <- rbind.pages(pages2)
# bind_rows(pages)
# APIData_All <- rbind.pages(c(pages[[1]],
#                              pages[[2]],
#                              pages[[3]]
#                             )
#                           )
...
tail(APIData_All, 3)
# View(APIData_All)
View(head(APIData_All, 3)
    )

str(Zips2)

LL_Stats_Test <- mutate(LL_Stats,
                        UniqueLatLng = paste(Lat_Med, Lng_Med, sep = "--")
                       )
View(head(LL_Stats_Test))

Zips2_Test <- mutate(Zips2,
                     UniqueLatLng = paste(lat, lng, sep = "--")
                    )
str(Zips2_Test)
View(head(Zips2_Test))

Combined_Test <- left_join(LL_Stats_Test,
                           Zips2_Test,
                           by = c("UniqueLatLng" = "UniqueLatLng")
                          )

View(Combined_Test)
View(filter(Combined_Test,
            between(RowNum, 1949, 2051) &
              is.na(adminCode2)
           )
    )
